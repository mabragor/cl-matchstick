    ...
    (flet ((default ()
	  ...
	  (let ((it (car pattern)))
	    (macrolet ((v (x)
			 `(string= ,(string x) it)))
	      (if (sym-not-kwd-p it)
		  (cond ((v collect-until) (codewalk-list-subpattern `(collect-while (not ,(second pattern)))))
			((v collect-while) (with-gensyms (g!-block)
					     `(let ((,g!-res nil))
						(block ,g!-block
						  (loop (handler-case (rebind-expr-next)
							  (fail-match () (return-from ,g!-block)))
